          ...
          <div class="field">
            <div class="control">
              <label class="block font-semibold text-sm" for="name"
                >Diffferential diagnose name</label
              >
              <input
                id="name"
                name="name"
                ...
                type="text"
                autofocus
                placeholder="Differential diagnose name..."
              />
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async add(newModel) {
      ...
      try {
        axios
          .post(`${serverUrl}/admin/differential-diagnosis`, newModel, {
            headers: {
          ...
          .then(() => {
            ...
            this.$router.push({ name: "differential-diagnose-index" });
          });
      } catch (error) {
    ...
    async update(id, newModel) {
      ...
      try {
        axios
          .put(`${serverUrl}/admin/differential-diagnosis/${id}`, newModel, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
