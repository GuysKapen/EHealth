          ...
          <div class="field">
            <div class="control">
              <label class="block font-semibold text-sm" for="name"
                >Provisional diagnose name</label
              >
              <input
                id="name"
                name="name"
                ...
                type="text"
                autofocus
                placeholder="Provisional diagnose name..."
              />
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async add(newModel) {
      ...
      try {
        axios
          .post(`${serverUrl}/admin/provisional-diagnosis`, newModel, {
            headers: {
          ...
          .then(() => {
            ...
            this.$router.push({ name: "provisional-diagnose-index" });
          });
      } catch (error) {
    ...
    async update(id, newModel) {
      ...
      try {
        axios
          .put(`${serverUrl}/admin/provisional-diagnosis/${id}`, newModel, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
