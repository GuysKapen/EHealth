...
export default {
  ...
  methods: {
    ...
    async add(newModel) {
      ...
      try {
        axios
          .post(`${serverUrl}/admin/comorbidities`, newModel, {
            headers: {
          ...
          .then(() => {
            ...
            this.$router.push({ name: "comorbidity-index" });
          });
      } catch (error) {
    ...
    async update(id, newModel) {
      ...
      try {
        axios
          .put(`${serverUrl}/admin/comorbidities/${id}`, newModel, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
