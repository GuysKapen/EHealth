            ...
            <div class="flex justify-between">
              <div class="flex items-center">
                ...
                <span
                  ...
                  >{{ diagnosis.length }}</span
                >
              </div>

              <router-link
                :to="{ name: 'differential-diagnose-new' }"
                  ...
                  >
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="(diagnose, idx) in diagnosis" :key="idx">
                          ...
                          >
                            ...
                            <button
                              class="p-3 mx-1 rounded-lg hover:bg-indigo-600 hover:text-white text-gray-400"
                              type="button"
                              @click="deleteModel(diagnose._id)"
                            >
...
export default {
  mounted() {
    ...
    axios
      .get(`${serverUrl}/admin/differential-diagnosis`, {
        headers: {
  ...
  methods: {
    deleteModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore();
        axios
          .delete(`${serverUrl}/admin/differential-diagnosis/${id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
          ...
          .then(function () {
            self.diagnosis = self.diagnosis.filter((item) => item._id !== id);
            createToast("Success delete DD", { type: "success" });
          });
      });
    },
  },
