            ...
            <div class="flex justify-between">
              <div class="flex items-center">
                ...
                <span
                  ...
                  >{{ comorbidities.length }}</span
                >
              </div>

              <router-link
                :to="{ name: 'comorbidity-new' }"
                  ...
                  >
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr
                          v-for="(specialization, idx) in comorbidities"
                          :key="idx"
                        >
...
export default {
  mounted() {
    ...
    axios
      .get(`${serverUrl}/admin/comorbidities`, {
        headers: {
  ...
  methods: {
    deleteModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore();
        axios
          .delete(`${serverUrl}/admin/comorbidities/${id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
          ...
          .then(function () {
            self.comorbidities = self.comorbidities.filter(
              (item) => item._id !== id
            );
            createToast("Success delete comorbidity", { type: "success" });
          });
      });
    },
  },
