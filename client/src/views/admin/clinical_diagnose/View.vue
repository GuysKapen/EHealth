                  ...
                  >
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="(specialization, idx) in CD" :key="idx">
...
export default {
  mounted() {
    ...
    axios
      .get(`${serverUrl}/admin/clinical-diagnosis`, {
        headers: {
  ...
  methods: {
    deleteModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore();
        axios
          .delete(`${serverUrl}/admin/clinical-diagnosis/${id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
          ...
          .then(function () {
            self.CD = self.CD.filter((item) => item._id !== id);
            createToast("Success delete CD", { type: "success" });
          });
      });
    },
  },
