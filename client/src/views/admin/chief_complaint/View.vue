            ...
            <div class="flex justify-between">
              <div class="flex items-center">
                ...
                <span
                  ...
                  >{{ chiefComplaint.length }}</span
                >
              </div>

              <router-link
                :to="{ name: 'chief-complaint-new' }"
                  ...
                  >
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr
                          v-for="(chiefComplaint, idx) in chiefComplaint"
                          :key="idx"
                        >
                          ...
                          >
                            ...
                            <button
                              class="p-3 mx-1 rounded-lg hover:bg-indigo-600 hover:text-white text-gray-400"
                              type="button"
                              @click="deleteModel(chiefComplaint._id)"
                            >
...
export default {
  mounted() {
    ...
    axios
      .get(`${serverUrl}/admin/chief-complaint`, {
        headers: {
  ...
  methods: {
    deleteModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore();
        axios
          .delete(`${serverUrl}/admin/chief-complaint/${id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
          ...
          .then(function () {
            self.chiefComplaint = self.chiefComplaint.filter(
              (item) => item._id !== id
            );
            createToast("Success delete chief complaint", { type: "success" });
          });
      });
    },
  },
