            ...
            <div class="flex justify-between">
              <div class="flex items-center">
                ...
                <span
                  ...
                  >{{ investigations.length }}</span
                >
              </div>

              <router-link
                :to="{ name: 'investigation-new' }"
                  ...
                  >
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr
                          v-for="(investigation, idx) in investigations"
                          :key="idx"
                        >
                          ...
                          >
                            ...
                            <button
                              class="p-3 mx-1 rounded-lg hover:bg-indigo-600 hover:text-white text-gray-400"
                              type="button"
                              @click="deleteModel(investigation._id)"
                            >
...
export default {
  mounted() {
    ...
    axios
      .get(`${serverUrl}/admin/investigations`, {
        headers: {
  ...
  methods: {
    deleteModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore();
        axios
          .delete(`${serverUrl}/admin/investigations/${id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
          ...
          .then(function () {
            self.investigations = self.investigations.filter(
              (item) => item._id !== id
            );
            createToast("Success delete investigation", { type: "success" });
          });
      });
    },
  },
