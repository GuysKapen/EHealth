                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        >
                          ...
                          <td
                            class="px-6 py-4 whitespace-nowrap text-center flex justify-end text-sm font-medium"
                          >
                            <router-link
                              :to="{
                            ...
                            <button
                              class="p-3 mx-1 rounded-lg hover:bg-indigo-600 hover:text-white text-gray-400"
                              type="button"
                              @click="deleteModel(appointment._id)"
                            >
...
export default {
  mounted() {
    ...
    axios
      .get(`${serverUrl}/users/${authStore.user._id}/appointments`, {
        headers: {
  ...
  methods: {
    deleteModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore();
        axios
          .delete(`${serverUrl}/doctor/appointments/${id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
          ...
          .then(function () {
            ...
            createToast("Success delete appointment", { type: "success" });
          });
      });
    },
    approveModel(id) {
      ...
      showConfirmPopup(
        function () {
          ...
          axios
            .put(
              `${serverUrl}/doctor/appointments/${id}/approve`,
              { status: true },
              {
                headers: {
                  "Content-Type": "application/json",
            ...
            .then(function () {
              self.appointments = self.appointments.map((item) =>
                item._id === id ? Object.assign(item, { status: true }) : item
              );
              createToast("Success approve appointment", { type: "success" });
            });
        },
    ...
    downloadModel(id) {
      const authStore = useAuthStore();
      window.open(
        `${serverUrl}/doctor/appointments/${id}/download?token=${authStore.token}`
      );
    },
  },
};
